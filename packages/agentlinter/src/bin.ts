...
// upload removed in @agenticapps fork (D-5-21): no network code, no telemetry
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    // --local / --no-share: argv-compat no-op (share feature removed in @agenticapps fork)
    else if (arg === '--local' || arg === '--no-share') { /* no-op */ }
  ...
  try {
    ...
    // Share / upload block removed in @agenticapps fork (D-5-21).
    // No network code remains in this CLI — output is local-only.
