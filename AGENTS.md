# AGENTS.md — agenticapps-dashboard

This project uses the AgenticApps spec-first workflow on the OpenAI
Codex CLI host. Product guarantees live in `openspec/specs/`, active
work lives in `openspec/changes/`, and archived changes preserve history.

The installed workflow version is recorded at
`.codex/workflow-version.txt`; project-specific configuration is in
`.codex/workflow-config.md` and `.planning/config.codex.json`.
...
holds in-flight deltas; `changes/archive/` is history.
...
Read `.codex/session-handoff.md` at session start if newer than 7 days;
write it before ending a session. Only the codex handoff — never another
host's. Full protocol in the trigger skill.
