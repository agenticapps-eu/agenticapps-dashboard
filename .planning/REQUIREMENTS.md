...
- [x] **COV-12**: Migration `0008-coverage-matrix-page.md` ships in `~/Sourcecode/agenticapps/claude-workflow/migrations/` alongside the dashboard PR; frontmatter `from_version: 1.7.0`, `to_version: 1.8.0`; documents the new `/coverage` route as a workflow surface (so other repos can discover where coverage is tracked) and bumps the agentic-apps-workflow skill version.
