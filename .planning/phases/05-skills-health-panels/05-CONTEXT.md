...
- **D-5-21 (added 2026-05-08, post-phase /cso): Spawn target is the LOCAL bin from a team-controlled `@agenticapps/agentlinter` fork, NEVER bare `npx agentlinter` against the open registry.** /cso surfaced that `npx --yes agentlinter` resolves to a third-party package owned by `simonkim <seojoon.kim@gmail.com>` — every cache miss could fetch a different version (no pin, no integrity check, install scripts auto-run). Two compromise paths: (a) account takeover → patch version pushes RCE to every dashboard host on next SkillHealth render; (b) ownership transfer → next release exfiltrates creds. Mitigation: fork upstream `seojoonkim/agentlinter` to `agenticapps-eu/agentlinter`, publish under `@agenticapps/agentlinter`, add as a regular pnpm dep so `pnpm-lock.yaml` pins the integrity hash. `agentLinterRunner.ts` resolves the bin via `createRequire` + `pkg.bin` and spawns `node <resolved-bin>`. Test #1 asserts the spawn cmd is `node` and arg[0] is a path under `@agenticapps/agentlinter` — fails CI if the spawn target ever regresses to `npx`. Upstream-watch GitHub Action lives in the fork (`docs/agentlinter-fork-upstream-watch.yml` is the source-of-truth template) and opens a tracking issue weekly when upstream commits or npm publishes diverge from the fork. D-5-15's "not-installed" state stays in the discriminated union — fires only on broken `node_modules` / partial install, never on package-not-found-in-registry. Updates the user-facing copy: "Reinstall with `pnpm install --frozen-lockfile`" (was: `npm install -g agentlinter`). Security report: `.gstack/security-reports/2026-05-08T04-50-46Z.json` Finding #1 (CRITICAL → resolved by this decision).
