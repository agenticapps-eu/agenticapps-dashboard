...
| Frontend stack | React + Vite SPA (Cloudflare Pages) |
| Database | none |
| LLM provider | none |
...
|---|---|---|
| `opencode-design-critique` quality bar | ≥ 90 | 80 |
| `opencode-impeccable-audit` quality bar | ≥ 90 | 80 |
| `opencode-qa` viewport widths | 1280, 390 | 1440, 390 |
| `opencode-database-sentinel-audit` blocking severity | Critical, High | Critical, High |
...
Project-defined override (if `TypeScript` is "Other"):

```
n/a — the TypeScript row applies.
```
