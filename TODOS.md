...
- ~~**[INFORMATIONAL] (Adversarial F14, confidence 6/10)** — `delta14d` wire field name locks in a 14-day window. Future 30d/60d toggles break the schema. Consider renaming to `deltaBaseline` + `baselineDays: number` for v1.3+.~~
  ...
  - **CLOSED 2026-05-21** by `feat/conformance-delta-baseline-rename`. Wire-format rename: `delta14d` → `deltaBaseline`, plus new `baselineDays: number` (z.number().int().positive()) carrying the window size. Future 30d/60d toggles change `BASELINE_DAYS` constant in conformanceScan.ts, not the schema. No back-compat shim — daemon and SPA ship together. FamilyCard prop also renamed: `delta14d` → `delta`, with new `baselineDays` prop driving the "{N}d trend" label dynamically. 14 files touched across shared/agent/SPA (schema + scan + cache test + registry test + fix-path route test + conformance route test + scan test + queries test + queries lib + FamilyCard + ConformancePage + their tests). Workspace: 2244/2244 tests pass. The two intentional `delta14d` mentions that survive in `conformanceScan.ts` are rename-history comments, not code.
