...
with two red cells is worse than one with none, and the reason is legible without
a formula. Sorting — by count of `fail`, then `never` — recovers the one thing a
score was actually used for, which is ordering the list.
...
from forty commits ago is not an assurance. The comparison is
`artifact mtime >= last commit touching production code` — **not** repo HEAD.
Comparing against HEAD makes every documentation edit invalidate every review,
and a state that is always true carries no information. The exclusion list
(`docs/`, `.planning/`, `openspec/`, root-level `*.md`) is therefore part of the
