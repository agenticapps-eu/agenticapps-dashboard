...
The scoring rules matter more than the presentation: cells that do not apply must
leave the denominator entirely, and repos that are drifted must not be counted as
failures of the tooling they may well have.
...
weighting every tracked column equally across every counted repo. Cells in the
`not-applicable` state MUST be excluded from both the numerator and the
denominator. Scores SHALL be integers.

#### Scenario: Not-applicable cells leave the denominator
- **WHEN** a repo has a column in the `not-applicable` state because the tool is not installed
- **THEN** that cell contributes to neither the numerator nor the denominator
- **AND** the resulting score is not depressed by a tool the fleet does not use.
